...
def save(f,a):
    ...
    #for list call recursion by calling list element
        if type_flag == 'L':
            ...
            f.write(f'{type_flag} {len(current_list_item)}\n')
            save(f,element)
...
def load(f):
    ...
    for line in f:
        ...
        if line[0] == 'S':
            ...
            lst_to_store += [int(line[2:])]
        elif line[0] == 'F':
            lst_to_store += [float(line[2:])]
        elif line[0] == 'L':
            nested_lst=[]
            for idx in range(0,int(line[2:]),1):
                nested_lst += load(f)
            lst_to_store.append(nested_lst)
            nested_lst=[]

            print(nested_lst)
            #print(int(line[2:-1])-1)
        print(lst_to_store[0])
        return lst_to_store
